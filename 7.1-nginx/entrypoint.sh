...
if [ "$1" = "php-fpm" ]; then
    ...
    sed -i "s^error_log = /proc/self/fd/2^;error_log = /proc/self/fd/2^" /etc/php/7.1/fpm/pool.d/www.conf
    sed -i "s^access.log = /proc/self/fd/2^;access.log = /proc/self/fd/2^" /etc/php/7.1/fpm/pool.d/www.conf
    sed -i "s^listen = 0\.0\.0\.0\:9000.*^listen = /run/php/php-fpm.sock^" /etc/php/7.1/fpm/pool.d/www.conf
